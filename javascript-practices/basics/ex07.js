...
console.log(math.add(10, -20), math.PI); // 본인 것부터 찾는데 본인 것이 생겼으니 본인 method로 실행한 결과를 출력
...
console.log(math.toString(), math.hasOwnProperty('add')); // hasOwnProperty -> 해당 속성이 있는지 확인하는 함수
math.toString = function() {
    return `[MyMath ${this.name}]`; // ES6에서 등장하는 literal template. 문자열로 출력되지만 내부에서 변수를 사용할 수 있다
}
...
console.log(math.toString.apply({name:'ㅋㅋㅋㅋ'}), math.add.call(null, 10, -20)); // apply -> parameter가 없는 함수를 호출하고 싶을 때 쓴다
